...
if %4 == "ATmega328"    SET AVRDUDE_PART_ID=m328
...
SET AVRDUDE_PATH=c:\Programmi\AVRdude
SET PATH=%PATH%;%AVRDUDE_PATH%\bin
SET AVRDUDE_CONF=%AVRDUDE_PATH%\avrdude.conf
SET AVRDUDE_PROGRAMMER=usbasp
SET AVRDUDE_OPTIONS=-b 115200
SET COMPORT=usb

@echo on

@rem *** Flash the AVR ***
%AVRDUDE_PATH%\avrdude.exe -C "%AVRDUDE_CONF%" %AVRDUDE_OPTIONS% -p %4 -c %AVRDUDE_PROGRAMMER% -P %COMPORT% -U flash:w:"%~nx2.hex"
